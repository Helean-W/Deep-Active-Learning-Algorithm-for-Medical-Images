...
class UNext(nn.Module):
    ...
    def __init__(self, num_classes, input_channels=3, deep_supervision=False, img_size=224, patch_size=16, in_chans=3,
        ...
        self.classHead = self.classificationHead()
        self.classavgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.classlinear = nn.Linear(32, 3)
    ...
    def forward(self, x):
        ...
        classification = self.classHead(out.detach())   # (B, 32, 4, 4)
        ...
        # classification = F.softmax(classification, dim=1)  在查询策略中有softmax
...
class UNext3(nn.Module):
    ...
    def __init__(self, num_classes, input_channels=3, deep_supervision=False, img_size=224, patch_size=16, in_chans=3,
        ...
        ###############
        self.seg_emb = nn.Conv2d(16, 16, kernel_size=1)
        ###############
    ...
    def forward(self, x):
        ...
        ###############
        out = self.seg_emb(out)
        ###############

        return self.final(out), out, classification, emb

    def get_embedding_dim(self):
        return 32   #

    def get_seg_channsels(self):
        return 16   #倒数第二层的输出通道数，也是最后一层的输入通道数，代表了每个像素的全连接的输入维度
...
if __name__ == '__main__':
    ...
    model = UNext3(1, 3, 1)
    output, out_emb, cla, em = model(input)
    print('output:', output.shape, 'output_emb:', out_emb.shape, 'class:', cla.shape, em.shape)   #[1, 1, 512, 512], [1, 3], [1, 32]
