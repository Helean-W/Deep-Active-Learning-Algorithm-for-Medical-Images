...
def parse_args():
    ...
    parser.add_argument('--name', default='UNext', help='model name')
...
def main():
    ...
    model = archs.__dict__['UNext'](1, 3, False)

    model = model.cuda()

    # Data loading code
    img_ids = glob(os.path.join('data', config['dataset'], 'test_images', '*' + config['img_ext']))
    ...
    val_dataset = Dataset(
        ...
        img_dir=os.path.join('data', config['dataset'], 'test_images'),
        mask_dir=os.path.join('data', config['dataset'], 'test_masks'),
