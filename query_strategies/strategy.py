...
class Strategy:
    ...
    def predict_prob(self, X, Y):
        loader_te = DataLoader(self.handler(X, Y, transform=self.args['transformTest']), shuffle=False,
    ...
    def get_grad_embedding(self, X, Y):
        ...
        loader_te = DataLoader(self.handler(X, Y, transform=self.args['transformTest']),
                               shuffle=False, **self.args['loader_te_args'])
        with torch.no_grad():
            for x, y, idxs in loader_te:  #torch.size([B, 3, 32, 32]), torch.size([1000]), torch.size([1000])
                # print('x, y, id', idxs)
                x, y = Variable(x.cuda()), Variable(y.cuda())
                cout, out = self.clf(x)
                # print(cout.shape, 1000, 10)
                # print(out.shape, 1000, 128)
