...
class CoreSet(Strategy_Unext):
    def __init__(self, X, Y, idxs_lb, handler, val_transform, tor=1e-4):
        super(CoreSet, self).__init__(X, Y, idxs_lb, handler, val_transform)
        self.tor = tor
    ...
    def query(self, n, net):
        ...
        embedding = self.get_embedding(self.X, self.Y, net)
